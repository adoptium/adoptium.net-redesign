  ...
  return (
    ...
    >
      ...
      <tbody className="table-light">
        {results ? (
          results.map((pkg, i): string | JSX.Element => (
            <tr key={i}>
              <td className="table-secondary py-4 text-white align-middle w-20">
                ...
                <span>
                  {Math.floor(
                    (Date.now() - new Date(pkg.binary.timestamp).getTime()) /
                      (1000 * 60 * 60 * 24),
                  ) > 180 && (
                    <span className="text-warning">
                      ...
                      <TiWarning
                        ...
                        title="This build is over 180 days old."
                        size={25}
                        style={{ color: "##B33B3B" }}
                      />
                      Out of Date
              ...
              <td className="align-middle">
                <img
                  width="100px"
                  alt={`${pkg.binary.distribution} logo`}
                  src={getImageForDistribution(pkg.binary.distribution)}
                />
              </td>
              <td className="align-middle">{capitalize(pkg.binary.os)}</td>
              <td className="align-middle">{pkg.binary.architecture}</td>
              <td className="align-middle">
...
const BinaryTable = ({
  ...
  javaVersion,
  vendor,
}: DownloadProps): null | JSX.Element => {
  return (
    <tr key={checksum}>
