...
const LatestTemurin = (props): JSX.Element => {
  ...
  return (
    <div
      ref={ref}
      className={
        props.page === "home" ? "container hide-on-mobile" : "container"
      }
    >
      {binary ? (
        <>
          ...
          {isSafari && (
            <span>
              We have detected that you're using Safari. Because we are{" "}
              <a
                target="_blank"
                rel="noopener noreferrer"
                href="https://stackoverflow.com/a/65412357"
              >
                unable to detect your architecture
              </a>{" "}
              we have suggested aarch64 (M1/M2). For x64 builds please see other{" "}
              <Link to="/temurin/releases">downloads</Link>.
            </span>
          )}
        </>
      ) : (
        ...
        {binary ? (
          <>
            <Link
              ...
              className="btn btn-lg btn-primary mt-3 py-3 text-white"
            >
              <FaDownload /> <Trans>Latest LTS Release</Trans>
              <br />
              <span style={{ fontSize: ".6em" }} className="font-weight-light">
                {binary.release_name}
              </span>
            </Link>
          </>
