...
describe("VersionSelector", () => {
  ...
  it("renders the component with default values", async () => {
    ...
    expect(screen.getByTestId("version-filter")).toHaveValue(
      defaultVersion.toString(),
    )
    expect(screen.queryByLabelText("View")).not.toBeInTheDocument()
    expect(
      screen.queryByLabelText("nightly builds prior to:"),
  ...
  it('renders the component with releaseType "ea"', async () => {
    ...
    await act(async () => {
      render(
        <VersionSelector updater={updater} releaseType="ea" Table={Table} />,
      )
    })
    expect(screen.getByTestId("version-filter")).toHaveValue(
      defaultVersion.toString(),
  ...
  it("updates the version when the select input changes", async () => {
    ...
    await act(async () => {
      fireEvent.change(screen.getByTestId("version-filter"), {
        target: { value: "2" },
      })
    })
  ...
  it("updates the number of builds and build date when the inputs change", async () => {
    ...
    fireEvent.change(screen.getByTestId("build-num-filter"), {
      target: { value: 10 },
    })
