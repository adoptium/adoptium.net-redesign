...
const ReleaseSelector: React.FC<ReleaseSelectorProps> = ({
  ...
  const handleOSChange = (os: string) => {
    ...
    if (os !== 'any' && selectedArch && selectedArch !== 'any' &&
        !osToArchMap[os]?.includes(selectedArch)) {
      setSelectedArch('any');
      updateArch('any');
    }
  };
  ...
  // Get base lists
  const osList = fetchOses(true)
  const archList = fetchArches(true)
  ...
  const filteredArchList = useMemo(() => {
    ...
    return archList.filter(arch => osToArchMap[selectedOS]?.includes(arch.value));
