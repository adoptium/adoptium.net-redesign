...
const Tabs = ({ updaterAction, Table }) => {
  ...
  const queryStringParams = queryString.parse(useLocation().search);
    ...
    if (versionParam) {
        ...
        } else if(LTSVersions.findIndex(version => version.node.version === nvp) >= 0) {
  ...
  useEffect(() => {
      (async () => {
        setReleases(await updaterAction(version, os, arch, packageType));
      })();
  }, [version, os, arch, packageType]);
  ...
  return (
    ...
    <section className="py-8 md:pt-16 px-6 w-full">
      <div className="w-full flex flex-col items-start justify-start sm:items-center sm:justify-center">
        <VersionSelector active={active} setActive={setActive} versions={LTSVersions} />
        {active === 1 ? <ReleaseSelector versions={data.allVersions} /> : <ButtonContent />}
      </div>
    </section>
    <Heading />
